...
// Objective
// In this assignment we develop a system for multiple terminals which will be interact with each other using message queues.
// The commands on a given terminal (shell) are mirrored in the coupled terminal along with output
...
int main()
{
    ...
    {
        // Execute bash commands in the parent process
        int uncpl = 1;
        ...
        while(1)
        {
            ...
            if(cnt > 0) fprintf(fp,"%s",input1);    // Print the command in history file
